...
async fn spawn_server(secret: Option<&str>) {
    tokio::spawn(Server::new(1024, 7835, secret).listen());
...
async fn spawn_client(secret: Option<&str>) -> Result<(TcpListener, SocketAddr)> {
    ...
    let client = Client::new("localhost", local_port, 7835,"localhost", 0, secret).await?;
...
async fn invalid_address() -> Result<()> {
    ...
    async fn check_address(to: &str, use_secret: bool) -> Result<()> {
        match Client::new("localhost", 5000, 7835, to, 0, use_secret.then_some("a secret")).await {
...
async fn very_long_frame() -> Result<()> {
    ...
    let mut attacker = TcpStream::connect(("localhost", 7835)).await?;
